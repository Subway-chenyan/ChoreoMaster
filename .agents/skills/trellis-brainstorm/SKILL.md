...
```bash
TASK_DIR=$(uv run python ./.trellis/scripts/task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
```bash
# Create child tasks
CHILD1=$(uv run python ./.trellis/scripts/task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(uv run python ./.trellis/scripts/task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
uv run python ./.trellis/scripts/task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
