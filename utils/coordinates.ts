...
export function mapTo3D(
  ...
  // In 3D: -z is front (towards camera), +z is back (away from camera)
  const z3d = ((pos.y - 50) / 50) * (config.depth / 2);

  // z: height in meters (vertical in Three.js, Y axis)
  const y3d = pos.z || 0;
...
export function mapTo2D(
  ...
  // y: -z is front, +z is back → 0-100 (0 is top/back, 100 is bottom/front)
