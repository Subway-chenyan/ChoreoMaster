    ...
    <div className="w-72 bg-slate-900 border-l border-slate-700 flex flex-col h-full overflow-y-auto">
      ...
      <div className="p-4 space-y-6">
        <div className="space-y-2">
          ...
          <input type="text" value={performer.name} onChange={(e) => onUpdatePerformer(performer.id, { name: e.target.value })} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm text-white" />
        </div>
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs font-medium text-slate-400"><Move3d size={14} /> 位置 (2D + 高度)</div>
          <div className="grid grid-cols-3 gap-2">
            <div className="space-y-1">
              <label className="text-xs text-slate-500 text-center block">X %</label>
              <EditableNumberInput step={0.1} value={position.x} onChange={(value) => handlePosChange('x', value)} formatValue={formatDecimal} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-center text-white" />
            </div>
            <div className="space-y-1">
              <label className="text-xs text-slate-500 text-center block">Y %</label>
              <EditableNumberInput step={0.1} value={position.y} onChange={(value) => handlePosChange('y', value)} formatValue={formatDecimal} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-center text-white" />
            </div>
            <div className="space-y-1">
              <label className="text-xs text-slate-500 text-center block">Z 米</label>
              <EditableNumberInput step={0.1} value={position.z || 0} onChange={(value) => handlePosChange('z', value)} formatValue={formatDecimal} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-center text-white" />
            </div>
          </div>
          <div className="space-y-1">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs font-medium text-slate-400"><RotateCw size={14} /> 旋转</div>
