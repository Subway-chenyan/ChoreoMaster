...
export const NoteItemRow: React.FC<NoteItemRowProps> = ({ item, onUpdate, onDelete }) => {
  return (
    <div className="flex items-start gap-2 p-2 rounded-md bg-slate-800/50 border border-slate-700/50 group">
      <div className="flex-1 min-w-0 space-y-1.5">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="物品/事件名称"
            className="flex-1 text-xs bg-slate-900 border border-slate-700 rounded px-2 py-1 text-slate-200 placeholder:text-slate-600 focus:border-blue-500 focus:outline-none"
          />
        ...
        <input
          ...
          placeholder="补充说明（可选）"
          className="w-full text-[11px] bg-transparent border-none text-slate-400 placeholder:text-slate-600 focus:outline-none"
        />
      </div>
      <button
        onClick={onDelete}
