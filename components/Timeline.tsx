...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  useEffect(() => {
    ...
    ctx.clearRect(0, 0, totalWidth, 160);
    ...
    for (let i = 0; i < totalWidth; i++) {
      ...
      ctx.fillRect(i, 80 + min * amp, 1, Math.max(1, (max - min) * amp));
    }
    ctx.globalAlpha = 1.0;
    ...
    >
      ...
      <div className="h-10 flex items-center px-4 bg-slate-900 border-b border-slate-800 justify-between z-20 relative">
        <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 ml-2">(Spacebar)</span>
        </div>
        <div className="flex items-center gap-4">
            <button 
                ...
                title="Add Formation at Current Time"
            >
      ...
      <div 
        className={`h-40 overflow-x-auto overflow-y-hidden relative custom-scrollbar bg-slate-950 ${isScrubbing ? 'cursor-col-resize' : 'cursor-default'}`} 
        ref={containerRef}
            ...
            <div className="absolute top-6 bottom-0 left-0 right-0 flex items-center">
                ...
                {frames.map((frame) => (
                    ...
                    >
                        ...
                        >
                            <div className="font-bold text-xs text-slate-200 truncate px-2 pointer-events-none mb-1">{frame.name}</div>
                            <div className="text-[9px] text-slate-400 pointer-events-none">{(frame.duration/1000).toFixed(1)}s</div>
