...
def _get_task_status(trellis_dir: Path, input_data: dict) -> str:
    ...
    if not active.task_path:
        return (
            ...
            "to clarify requirements and create a task via `uv run python ./.trellis/scripts/task.py create`.\n"
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            f"Source: {active.source}\n"
            f"Next-Action: Run `uv run python ./.trellis/scripts/task.py finish` to clear the stale pointer, "
    ...
    if task_status == "completed":
        return (
            ...
            f"then archive with `uv run python ./.trellis/scripts/task.py archive {task_dir.name}`."
        )
    ...
    if implement_jsonl.is_file() and not _has_curated_jsonl_entry(implement_jsonl):
        return (
            ...
            "`uv run python ./.trellis/scripts/get_context.py --mode packages` to list available specs, "
            "then edit the jsonl files or use `uv run python ./.trellis/scripts/task.py add-context`. "
